...
| Variable | Description | Source |
| :--- | :--- | :--- |
| `n_housing_units` | Number of active housing units (HUs) at the facility as of December 2025 — the report's own term ("Number of Housing Units (HU)"). HUs are wings, dormitories, or cell tiers. Denominator for `pct_hu_*`. CVSP and FWF are null (not in the CDCR Jan 2026 report). | CDCR Air Cooling Pilot Program Supplemental Report, January 2026 (Table 2) |
| `pct_hu_mechanical` | Proportion of housing units (HUs) with mechanical (refrigerant) cooling — real AC. Used by the heat-risk index (AC sub-indicator = `1 − pct_hu_mechanical`) and the app cooling mix. | CDCR Air Cooling Pilot Program Supplemental Report, January 2026 (Table 2) |
| `pct_hu_evaporative` | Proportion of housing units (HUs) with evaporative (swamp) cooling. | CDCR Air Cooling Pilot Program Supplemental Report, January 2026 (Table 2) |
| `pct_hu_air_handlers` | Proportion of housing units (HUs) with air handlers only (ventilation, no cooling). | CDCR Air Cooling Pilot Program Supplemental Report, January 2026 (Table 2) |
...
> **Cooling data.** The `pct_hu_*` columns are the housing-unit cooling mix (mechanical / evaporative / ventilation-no-cooling) from the **CDCR Air Cooling Pilot Supplemental Report (Jan 2026, as of Dec 2025)** — the newest, complete, authoritative per-facility inventory. Mixed-cooling units are counted under each type, so the three shares can sum slightly > 1 (e.g. SATF evaporative = 1.03).
>
> **History (2026-07).** Two cleanups landed together. (1) An older Reuters FOIA (2025) HVAC-equipment inventory had supplied a parallel `pct_units_*` set plus an equipment-level count also called `n_housing_units`. It was incomplete — it missed housing at 11 of 31 facilities (e.g. CIM: 15 equipment rows → a spurious 100% refrigeration vs the report's ~43%) — so the whole Reuters set was **dropped**. (2) The CDCR columns, previously mis-named `pct_buildings_*` / `n_housing_buildings`, were **renamed** to `pct_hu_*` / `n_housing_units` to match the report's "Housing Units" terminology. So `n_housing_units` now refers to the CDCR housing-unit count, not the removed Reuters equipment count. The frozen heat-operations regression reads `n_housing_units`; that name now resolves to the CDCR count (a valid but different value than the Reuters count it was written against), so it should be reviewed if revived.
>
> Validated against CDCR's June-2025 statewide pie (evaporative 52% / mechanical 24% / air-handlers 19% / fans 5%): the report matches on mechanical (23%); its lower evaporative / higher "ventilation only" reflects real degradation of swamp-cooling systems by Dec 2025, not a labeling error. One per-facility caveat: CIM reads 0% evaporative even though its Facility A retrofit (completed Feb 2025) is evaporative per CEQA #2018128257 — CIM's mix is the least certain.
