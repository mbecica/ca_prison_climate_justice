...
md_lines = [
    ...
    f'### Hazard Component',
    f'',
    f'A blended daytime term and a warm-night term, each max-normalized 0–1 across 31 facilities and averaged, then multiplied by an air-quality modifier. Temperature counts come from a LOCA2-CA daily extraction at each facility\'s grid cell (14-model ensemble, model democracy).',
    f'',
    f'| Sub-indicator | Description | Research justification | Source |',
    f'|:---|:---|:---|:---|',
    f'| Hot days (blended) | 50/50 blend of a facility-relative threshold (days above the facility\'s mean summer daily max + 10°F, 1981–2010; `loca2_days_over_avg_plus10`) and an absolute 90°F count (`loca2_days_over_90`), each max-normalized first | The +10°F relative offset is calibrated to the mortality estimate in Skarha et al. (2023); the absolute 90°F count is the CDCR Heat Pathology Plan Stage I trigger | LOCA2-CA daily (SSP3-7.0), Cal-Adapt via cadcat |',
...
md_lines += [
    ...
    f'',
    f'A blended daytime term and a warm-night term, each max-normalized 0–1 across 31 facilities and averaged, then multiplied by an air-quality modifier (`× (1 + 0.30·AQI/100)`):',
    f'',
    f'| Sub-indicator | Variable | Source |',
    f'|:---|:---|:---|',
    f'| Hot days (blended) | 50/50 `loca2_days_over_avg_plus10` (relative) ⊕ `loca2_days_over_90` (absolute) | LOCA2-CA daily (SSP3-7.0), Cal-Adapt via cadcat |',
