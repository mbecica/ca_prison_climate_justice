...
OUTCOMES = [
    ...
    ("violent_incidents",         "All Violent Incidents (raw)",       "violent_incidents",           "All Violent\nIncidents"),
    ...
    for period_label, mask_fn in [("Pre", lambda d: d["date"] < treat_date),
        ...
        for grp_label, grp_df in [(treated_fac, fac_df), ("Control (mean)", ctrl_df)]:
            ...
            row = {
                ...
                "Mean Staff-Involved": round(sub["staff_involved"].mean(), 1),
                "Mean All Violent": round(sub["violent_incidents"].mean(), 1),
...
def make_event_figure(treated_fac, treat_date):
    ...
    plot_specs = [
        ...
        ("inmate_on_inmate",           "Inmate-on-Inmate Violence\n(count/month)"),
        ("staff_involved",             "Staff-Involved Incidents\n(count/month)"),
